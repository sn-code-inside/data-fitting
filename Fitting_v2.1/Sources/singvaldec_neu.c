...
diff_of_products2( double a, double b, double c, double d )
{
	double cd = c * d;
	         ...
	         double** v )
{
	 ...
	 */
	for (int k = M - 1; k >= 0; k--)	/* loop over singular values */
	{
		assert( k >= 0);
		for (int its = 0; its < 30; its++)	/* loop over allowed
																			   iterations */
		{
			flag = 1;
			...
			if (flag)
			{
				...
				for (int i = l; i < k; i++)
				{
					...
					for (int j = 0; j < N; j++)
					{
						...
						a[j][nm] = fma( y, c, z * s );
						//a[j][i] = z * c - y * s;
						a[j][i] = diff_of_products2( z, c, y, s );
					}
				}
			}
			...
			for (int j = l; j <= nm; j++)
			{
				...
				for (jj = 0; jj < M; jj++)
				{
					...
					v[jj][j] = fma( x, c, z * s );
					//v[jj][i] = z * c - x * s;
					v[jj][i] = diff_of_products2( z, c, x, s );
				}
				z = euclid_dist( f, h);
				w[j] = z;
				...
				for (int jj = 0; jj < N; jj++)
				{
					...
					a[jj][j] = fma( y, c, z * s );
					//a[jj][i] = z * c - y * s;
					a[jj][i] = diff_of_products2( z, c, y, s );
				}
			}
			assert( l >= 0);
			assert( k >= 0);
